%%
function [epos, rotate_new] = ForKine_jointbound_approx(x,DH,base,RoCap, ys, lmd)
%% polishing 
% new 6 to end tool 
...
for i=1:nlink
    if i <= nlink
        R=[cossym(DH(i,1),ys(i)*lmd)    -sinsym(DH(i,1),ys(i)*lmd)*cos(DH(i,4))     sinsym(DH(i,1),ys(i)*lmd)*sin(DH(i,4));
            sinsym(DH(i,1),ys(i)*lmd)   cossym(DH(i,1),ys(i)*lmd)*cos(DH(i,4))   -cossym(DH(i,1),ys(i)*lmd)*sin(DH(i,4));
            0  sin(DH(i,4)) cos(DH(i,4))];
        T=[DH(i,3)*cossym(DH(i,1),ys(i)*lmd);    DH(i,3)*sinsym(DH(i,1),ys(i)*lmd);   DH(i,2)];
