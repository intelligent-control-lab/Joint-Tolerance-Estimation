...
for iter = 1:maximum_iter
    ...
    b = [];
    rng(seed);
    xref = [0.002 1000000*rand(1,1+nlink)]; % lmd b c d
    LB = [0, lb*ones(1,1+nlink)];
    UB = [0.1, inf*ones(1,1+nlink)];
    
    [x, fval, exitflag,output] = fmincon(obj,xref,[],[],A,b,LB,UB,@(x)nonlcon_hierarchy_fk_3dof_playaround(x,Q,Q1,Q2,Q3,Q_cons),options);
    toc
    
    if exitflag > 0
        % log the solution 
        % valid solution found 
        c = nonlcon_hierarchy_fk_3dof_playaround(x,Q,Q1,Q2,Q3,Q_cons);
