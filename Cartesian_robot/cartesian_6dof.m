...
nlink = 6;
c_pre = ForKine_sym_nolmd_casual_dof(theta_ini, robot.DH, robot.base, robot.cap, ys, nlink);
...
% xwall = epos(1) + 0.04 - 0.002; %it is 2cm away 
xwall = epos(1) + 0.04 - maximum_delta; % this wall becomes the lower bound
...
for iter = 1:maximum_iter
    ...
    b = [];
    rng(seed);
    xref = [0 1000000*rand(1,1+nlink)]; % lmd b c d
    LB = [0, lb*ones(1,1+nlink)];
    UB = [0.1, inf*ones(1,1+nlink)];
    
    [x, fval, exitflag,output] = fmincon(obj,xref,[],[],A,b,LB,UB,@(x)nonlcon_hierarchy_fk_6dof_playaround(x,Q,Q1,Q2,Q3,Q4,Q5,Q6,Q_cons),options);
    toc
    
    if exitflag > 0
        % log the solution 
        % valid solution found 
        c = nonlcon_hierarchy_fk_6dof_playaround(x,Q,Q1,Q2,Q3,Q4,Q5,Q6,Q_cons);
